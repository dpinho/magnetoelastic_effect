...
\int_s(\frac{\mathbf{B}}{\mu_0}-\mathbf{M})\cdot d\mathbf{s}-\frac{d}{dt}\int\int_S(\epsilon_0\,\textbf{E}+\mathbf{P})\cdot\textit{d}\textbf{A}&=\int\int_S\mathbf{J}_f\cdot\,d\mathbf{A},
\end{align*}
onde $\frac{\mathbf{B}}{\mu_0}-\mathbf{M}=\mathbf{H}$ \'e definido como o \textit{campo magn\'etico auxiliar}. Em atividades experimentais \'e mais f\'acil medir o campo 
...
\int_s\mathbf{H}\cdot d\mathbf{s}-\frac{d}{dt}\int\int_S\mathbf{D}\cdot\textit{d}\textbf{A}&=\int\int_S\mathbf{J}_f\cdot\,d\mathbf{A},\\\nonumber\\\label{eq.max_meio_4}
...
\end{equation*}
Matematicamente n\~ao podemos usar a arbitrariedade de escolha da superf\'icie $A$, mas resultados experimentais tem sustentado que
\begin{equation*}
